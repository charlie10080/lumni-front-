const API_BASE_URL = 'http://localhost:3000/api';
    ...
    // Estado global de trimestres (1, 2, 3 o 'anual')
    let selectedTrimester = 1;

    // Helpers de Fecha de Asistencia y Exclusión de Fines de Semana (Sábados y Domingos)
    ...
    function getNearestWeekday(dateStr, direction = -1) {
    ...
    // Fecha escolar seleccionada (si hoy es fin de semana, apunta al viernes hábil más cercano)
    let selectedAttendanceDate = isWeekend(getTodayIsoDate()) ? getNearestWeekday(getTodayIsoDate(), -1) : getTodayIsoDate();

    // Datos del Maestro & Suscripción Docente ($250 MXN inicial / $50 MXN al mes / $200 MXN renovación de ciclo con descuento)
    const SUSCRIPCION_INICIAL = 250;
    const SUSCRIPCION_MENSUAL = 50;
    const SUSCRIPCION_RENOVACION_DESCUENTO = 200; // Descuento de $50 MXN
    ...
    // Recalcular totales de asistencia excluyendo fines de semana
    function recalculateStudentAttendanceTotals(alumno) {
      if (!alumno.asistenciasPorFecha) alumno.asistenciasPorFecha = {};
      let pres = 0, ret = 0, falt = 0;
      Object.keys(alumno.asistenciasPorFecha).forEach(f => {
        if (isWeekend(f)) return; // Excluir sábados y domingos
        const record = alumno.asistenciasPorFecha[f];
        const status = typeof record === 'string' ? record : record?.status;
        if (status === 'presente') pres++;
        else if (status === 'retardo') ret++;
        else if (status === 'falta') falt++;
      });
      alumno.asistenciasTotales = { presentes: pres, retardos: ret, faltas: falt };
      return alumno.asistenciasTotales;
    }

    function ensureStudentTrimesterGrades(alumno) {
      if (!alumno.calificacionesTrimestres) {
        alumno.calificacionesTrimestres = { 1: {}, 2: {}, 3: {} };
      }
      if (!alumno.calificacionesTrimestres[1]) alumno.calificacionesTrimestres[1] = {};
      if (!alumno.calificacionesTrimestres[2]) alumno.calificacionesTrimestres[2] = {};
      if (!alumno.calificacionesTrimestres[3]) alumno.calificacionesTrimestres[3] = {};

      if (alumno.calificaciones && typeof alumno.calificaciones === 'object') {
        Object.keys(alumno.calificaciones).forEach(mat => {
          if (alumno.calificacionesTrimestres[1][mat] === undefined && alumno.calificaciones[mat] !== '') {
            alumno.calificacionesTrimestres[1][mat] = alumno.calificaciones[mat];
          }
        });
      }
    }
    ...
    // Limpieza e inicialización de asistencias por fecha y trimestres
    alumnosState.forEach(a => {
      if (!a.asistenciasPorFecha) a.asistenciasPorFecha = {};
      const today = getTodayIsoDate();
      if (a.asistenciaHoy && a.asistenciaHoy !== 'pendiente' && !a.asistenciasPorFecha[today] && !isWeekend(today)) {
        a.asistenciasPorFecha[today] = {
          status: a.asistenciaHoy,
          hora: a.horaAsistencia || '--:--'
        };
      }
      recalculateStudentAttendanceTotals(a);
      ensureStudentTrimesterGrades(a);
    });
    ...
    function renderParentPortal(alumno) {
      ...
      // 2. Asistencias (Excluyendo Fines de Semana)
      ensureStudentTrimesterGrades(alumno);
      recalculateStudentAttendanceTotals(alumno);
      ...
      const recordHoy = getStudentAttendanceForDate(alumno, getTodayIsoDate());
      if (recordHoy.status === 'presente') {
        hoyTexto = `Presente (${recordHoy.hora || 'A tiempo'})`;
      } else if (recordHoy.status === 'retardo') {
        hoyTexto = `Retardo (${recordHoy.hora || 'Tarde'})`;
      } else if (recordHoy.status === 'falta') {
      ...
      matContainer.innerHTML = materiasState.map(m => {
        const v1 = alumno.calificacionesTrimestres?.[1]?.[m];
        const v2 = alumno.calificacionesTrimestres?.[2]?.[m];
        const v3 = alumno.calificacionesTrimestres?.[3]?.[m];
        ...
        if (matCount === 0 && alumno.calificaciones?.[m] !== undefined && alumno.calificaciones?.[m] !== '') {
          matSum = parseFloat(alumno.calificaciones[m]);
          matCount = 1;
        }

        const cal = matCount > 0 ? (matSum / matCount).toFixed(1) : '-';
        if (matCount > 0) {
          sum += (matSum / matCount);
          count++;
        }
        ...
        return `
          <div class="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
            ...
            <div class="flex items-center gap-1.5">
              <span class="px-2.5 py-1 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700 font-extrabold ${matCount > 0 ? 'text-brand-700 dark:text-brand-300' : 'text-slate-400'} text-xs shadow-2xs">
                ${cal}
              </span>
            </div>
          </div>
    ...
    function getStudentAttendanceForDate(alumno, dateStr) {
      if (!alumno.asistenciasPorFecha) alumno.asistenciasPorFecha = {};
      const record = alumno.asistenciasPorFecha[dateStr];
      if (!record) {
        if (dateStr === getTodayIsoDate() && alumno.asistenciaHoy) {
          return {
            status: alumno.asistenciaHoy,
            hora: alumno.horaAsistencia || '--:--'
          };
        }
        return { status: 'pendiente', hora: '--:--' };
      }
      if (typeof record === 'string') {
        return { status: record, hora: '--:--' };
      }
      return {
    ...
    function setStudentAttendanceForDate(alumno, dateStr, status, customTime = null) {
      if (!alumno.asistenciasPorFecha) alumno.asistenciasPorFecha = {};
      if (isWeekend(dateStr)) {
        showToast("⚠️ Los fines de semana (sábados y domingos) no son días hábiles escolares.", "error");
        return;
      }
      const timeStr = customTime || new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      const finalHora = (status === 'presente' || status === 'retardo') ? timeStr : '--:--';
      
      alumno.asistenciasPorFecha[dateStr] = {
        status: status,
        hora: finalHora
      };

      if (dateStr === getTodayIsoDate()) {
        alumno.asistenciaHoy = status;
        alumno.horaAsistencia = finalHora;
      }
      recalculateStudentAttendanceTotals(alumno);
    }
    ...
    function onAttendanceDateChange(val) {
      if (!val) return;
      if (isWeekend(val)) {
        const adjusted = getNearestWeekday(val, -1);
        showToast("⚠️ Los sábados y domingos no son días lectivos. Se ajustó al día hábil más cercano.", "info");
        selectedAttendanceDate = adjusted;
        const input = document.getElementById('asist-selected-date');
    ...
    function changeAttendanceDate(delta) {
      const [y, m, d] = selectedAttendanceDate.split('-').map(Number);
      ...
      selectedAttendanceDate = `${yStr}-${mStr}-${dStr}`;
    ...
    function goToTodayAttendance() {
      const today = getTodayIsoDate();
      selectedAttendanceDate = isWeekend(today) ? getNearestWeekday(today, -1) : today;
      const input = document.getElementById('asist-selected-date');
      if (input) input.value = selectedAttendanceDate;
      updateAttendanceDateBadge();
      filterAttendanceTable();
    ...
    function updateAttendanceDateBadge() {
      ...
      const input = document.getElementById('asist-selected-date');
      if (input && input.value !== selectedAttendanceDate) {
        input.value = selectedAttendanceDate;
      }
      if (!badge) return;
      const [y, m, d] = selectedAttendanceDate.split('-').map(Number);
      const dateObj = new Date(y, m - 1, d);
      const options = { weekday: 'short', day: 'numeric', month: 'short' };
      const label = dateObj.toLocaleDateString('es-ES', options);
      const isToday = selectedAttendanceDate === getTodayIsoDate();
      badge.textContent = isToday ? `📅 Hoy (${label})` : `📅 ${label}`;
    }
    ...
    function handleAttendanceScan(scannedCode) {
      ...
      if (isWeekend(selectedAttendanceDate)) {
        playScanChime('error');
        showToast("No se puede registrar asistencia en fin de semana. Selecciona un día entre lunes y viernes.", "error");
        return;
      }
      ...
      setStudentAttendanceForDate(a, selectedAttendanceDate, 'presente', timeStr);

      document.getElementById('last-scan-name').textContent = a.nombre;
      document.getElementById('last-scan-meta').textContent = `Fecha: ${selectedAttendanceDate} • Hora: ${timeStr} • A tiempo`;
      document.getElementById('last-scan-feedback').classList.remove('hidden');

      saveState();
      showToast(`Asistencia confirmada para ${a.nombre} (${selectedAttendanceDate})`, "success");
      updateTeacherViews();
    }

    function markAllAttendance(status) {
      if (isWeekend(selectedAttendanceDate)) {
        showToast("Los fines de semana (sábados y domingos) no son días lectivos.", "error");
        return;
      }
      const timeStr = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
      alumnosState.forEach(a => {
        setStudentAttendanceForDate(a, selectedAttendanceDate, status, timeStr);
      });
      saveState();
      updateTeacherViews();
      showToast(`Todos marcados como ${status} el ${selectedAttendanceDate}`, "success");
    }
    ...
    function setAlumnoAttendance(uuid, status) {
      ...
      setStudentAttendanceForDate(a, selectedAttendanceDate, status);
      saveState();
      updateTeacherViews();
    ...
    function filterAttendanceTable() {
      ...
      const filtered = alumnosState.filter(a => {
        const record = getStudentAttendanceForDate(a, selectedAttendanceDate);
        const matchStatus = currentAsistFilter === 'todos' || record.status === currentAsistFilter;
    ...
    function renderAttendanceTable(customData = null) {
      ...
      tbody.innerHTML = data.map(a => {
        ...
        if (record.status === 'presente') badgeColor = 'bg-emerald-100 text-emerald-800 border border-emerald-200';
        if (record.status === 'falta') badgeColor = 'bg-rose-100 text-rose-800 border border-rose-200';
        if (record.status === 'retardo') badgeColor = 'bg-amber-100 text-amber-800 border border-amber-200';
        ...
        return `
          <tr class="hover:bg-slate-50 dark:bg-slate-800 transition-colors">
            ...
            <td class="px-4 py-3 font-mono text-xs text-slate-500 dark:text-slate-400">${record.hora}</td>
            <td class="px-4 py-3 text-right space-x-1">
              <button onclick="setAlumnoAttendance('${a.uuid}', 'presente')" title="Presente" class="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-900/40 hover:bg-emerald-100 text-emerald-700 dark:text-emerald-400 rounded-lg text-xs font-bold transition-all cursor-pointer ${record.status === 'presente' ? 'ring-2 ring-emerald-500' : ''}">P</button>
              <button onclick="setAlumnoAttendance('${a.uuid}', 'retardo')" title="Retardo" class="px-2.5 py-1 bg-amber-50 dark:bg-amber-900/40 hover:bg-amber-100 text-amber-700 dark:text-amber-400 rounded-lg text-xs font-bold transition-all cursor-pointer ${record.status === 'retardo' ? 'ring-2 ring-amber-500' : ''}">R</button>
              <button onclick="setAlumnoAttendance('${a.uuid}', 'falta')" title="Falta" class="px-2.5 py-1 bg-rose-50 dark:bg-rose-900/40 hover:bg-rose-100 text-rose-700 dark:text-rose-400 rounded-lg text-xs font-bold transition-all cursor-pointer ${record.status === 'falta' ? 'ring-2 ring-rose-500' : ''}">F</button>
            </td>
          </tr>
        `;
      }).join('');
    ...
    function switchTrimester(trim) {
      selectedTrimester = trim;
      [1, 2, 3, 'anual'].forEach(t => {
        const btn = document.getElementById(`trimestre-tab-${t}`);
        if (btn) {
          if (t === trim) {
            btn.className = "px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all bg-brand-600 text-white shadow-xs cursor-pointer";
          } else {
    ...
    function getStudentTrimesterGrade(alumno, trim, materia) {
      ensureStudentTrimesterGrades(alumno);
      if (trim === 'anual') {
        const v1 = alumno.calificacionesTrimestres[1]?.[materia];
        const v2 = alumno.calificacionesTrimestres[2]?.[materia];
        const v3 = alumno.calificacionesTrimestres[3]?.[materia];
        let sum = 0, count = 0;
        [v1, v2, v3].forEach(v => {
          if (v !== undefined && v !== null && v !== '') {
            sum += parseFloat(v) || 0;
      ...
      const val = alumno.calificacionesTrimestres[trim]?.[materia];
    ...
    function renderDynamicGradesTable() {
      ...
      const trimLabel = selectedTrimester === 'anual' ? 'Promedio Anual' : `Promedio ${selectedTrimester}° Trim`;
      ...
      headersHtml += `<th class="px-4 py-3 text-center cursor-pointer select-none" onclick="sortGrades('promedio')">${trimLabel} ${getSortIcon('promedio')}</th>`;
        ...
        data.sort((a, b) => {
          ...
          if (gradesSortCol === 'nombre') {
            ...
            const getProm = (al) => {
              let s = 0, c = 0;
              materiasState.forEach(m => {
                const g = getStudentTrimesterGrade(al, selectedTrimester, m);
                if (g !== null) {
                  s += g;
                  c++;
                }
            ...
            valA = getStudentTrimesterGrade(a, selectedTrimester, gradesSortCol) ?? -1;
            valB = getStudentTrimesterGrade(b, selectedTrimester, gradesSortCol) ?? -1;
          }
      ...
      tbody.innerHTML = data.map((alumno) => {
        ...
        let sum = 0, count = 0;
        materiasState.forEach(m => {
          if (selectedTrimester === 'anual') {
            const valNum = getStudentTrimesterGrade(alumno, 'anual', m);
            const valDisplay = valNum !== null ? valNum.toFixed(1) : '-';
            if (valNum !== null) {
              sum += valNum;
              count++;
            }
            rowHtml += `
              <td class="px-3 py-2 text-center">
                <span class="inline-block px-2.5 py-1 bg-slate-100 dark:bg-slate-800 font-mono font-bold text-xs rounded-lg text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                  ${valDisplay}
                </span>
              </td>
            `;
          } else {
            const valNum = alumno.calificacionesTrimestres[selectedTrimester]?.[m];
            const hasVal = valNum !== undefined && valNum !== null && valNum !== '';
            const val = hasVal ? Math.round(parseFloat(valNum) || 0) : '';
            if (hasVal) {
              sum += parseFloat(valNum) || 0;
              count++;
            }
            rowHtml += `
              <td class="px-3 py-2 text-center">
        ...
        rowHtml += `
          ...
          <td class="px-3 py-3 text-right">
            <button onclick="openBoletaModal('${alumno.uuid}')" class="px-2.5 py-1 bg-brand-50 dark:bg-brand-900/40 hover:bg-brand-100 text-brand-700 dark:text-brand-300 rounded-lg text-xs font-bold transition-all inline-flex items-center gap-1 cursor-pointer">
              <i data-lucide="file-text" class="w-3.5 h-3.5"></i>
              <span>Boleta</span>
            </button>
          </td>
    ...
    function updateDynamicGrade(alumnoIdx, materia, value) {
      ...
      const currentTrim = (selectedTrimester === 'anual') ? 1 : selectedTrimester;
      const trimmed = String(value).trim();
      if (trimmed === '') {
        delete alumno.calificacionesTrimestres[currentTrim][materia];
        if (currentTrim === 1) delete alumno.calificaciones[materia];
      } else {
        let intVal = parseInt(trimmed, 10);
        if (isNaN(intVal)) intVal = 0;
        if (intVal > 10) intVal = 10;
        if (intVal < 0) intVal = 0;
        alumno.calificacionesTrimestres[currentTrim][materia] = intVal;
        alumno.calificaciones[materia] = intVal;
      }

      let sum = 0, count = 0;
      materiasState.forEach(m => {
        const val = alumno.calificacionesTrimestres[currentTrim]?.[m];
        if (val !== undefined && val !== null && val !== '') {
    ...
    function renderBoletaModal(student) {
      ...
      ensureStudentTrimesterGrades(student);

      // Calificaciones por Asignatura (Desglose Trimestres 1, 2, 3 • Promedio Final)
      ...
      tbody.innerHTML = materiasState.map(m => {
        const v1 = student.calificacionesTrimestres?.[1]?.[m];
        const v2 = student.calificacionesTrimestres?.[2]?.[m];
        const v3 = student.calificacionesTrimestres?.[3]?.[m];

        const has1 = v1 !== undefined && v1 !== null && v1 !== '';
        const has2 = v2 !== undefined && v2 !== null && v2 !== '';
        const has3 = v3 !== undefined && v3 !== null && v3 !== '';

        if (has1) { sumT1 += parseFloat(v1); countT1++; }
        if (has2) { sumT2 += parseFloat(v2); countT2++; }
        if (has3) { sumT3 += parseFloat(v3); countT3++; }

        let matSum = 0, matCount = 0;
        if (has1) { matSum += parseFloat(v1); matCount++; }
        if (has2) { matSum += parseFloat(v2); matCount++; }
        if (has3) { matSum += parseFloat(v3); matCount++; }

        const matAvg = matCount > 0 ? (matSum / matCount) : null;
        if (matAvg !== null) {
          sumFinal += matAvg;
          countFinal++;
        }
        ...
        if (matAvg !== null) {
          if (matAvg < 6) rating = 'Insuficiente';
          else if (matAvg < 7.5) rating = 'Básico';
          else if (matAvg < 9) rating = 'Satisfactorio';
          else rating = 'Sobresaliente';
        }

        return `
          <tr class="hover:bg-slate-50 transition-colors">
            <td class="px-3.5 py-2.5 font-semibold text-slate-900">${m}</td>
            <td class="px-2 py-2.5 text-center font-mono font-bold text-slate-700">${has1 ? Math.round(v1) : '-'}</td>
            <td class="px-2 py-2.5 text-center font-mono font-bold text-slate-700">${has2 ? Math.round(v2) : '-'}</td>
            <td class="px-2 py-2.5 text-center font-mono font-bold text-slate-700">${has3 ? Math.round(v3) : '-'}</td>
            <td class="px-2 py-2.5 text-center font-mono font-black text-brand-900 bg-brand-50/40">${matAvg !== null ? matAvg.toFixed(1) : '-'}</td>
            <td class="px-3 py-2.5 text-center text-xs font-semibold ${matAvg !== null ? (matAvg >= 9 ? 'text-emerald-700' : (matAvg >= 7 ? 'text-indigo-700' : 'text-amber-700')) : 'text-slate-400'}">${rating}</td>
          </tr>
        `;
      }).join('');

      const avgT1 = countT1 > 0 ? (sumT1 / countT1).toFixed(1) : '-';
      const avgT2 = countT2 > 0 ? (sumT2 / countT2).toFixed(1) : '-';
      const avgT3 = countT3 > 0 ? (sumT3 / countT3).toFixed(1) : '-';
      const avgFinal = countFinal > 0 ? (sumFinal / countFinal) : null;

      const elAvgT1 = document.getElementById('boleta-avg-t1');
      if (elAvgT1) elAvgT1.textContent = avgT1;
      const elAvgT2 = document.getElementById('boleta-avg-t2');
      if (elAvgT2) elAvgT2.textContent = avgT2;
      const elAvgT3 = document.getElementById('boleta-avg-t3');
      if (elAvgT3) elAvgT3.textContent = avgT3;

      document.getElementById('boleta-final-average').textContent = avgFinal !== null ? avgFinal.toFixed(1) : '-';
      let finalRating = 'Sin evaluar';
      if (avgFinal !== null) {
        if (avgFinal < 6) finalRating = 'Insuficiente';
        else if (avgFinal < 7.5) finalRating = 'Básico';
        else if (avgFinal < 9) finalRating = 'Satisfactorio';
        else finalRating = 'Sobresaliente';
      }
      document.getElementById('boleta-final-rating').textContent = finalRating;

      // Resumen de Asistencias (Excluyendo Fines de Semana)
      recalculateStudentAttendanceTotals(student);
    ...
    function populateReportesSelect() {
      ...
      const editSelect = document.getElementById('edit_reporte_alumno_uuid');
      if (alumnosState.length === 0) {
        if (select) select.innerHTML = '<option value="">Sin alumnos</option>';
        if (editSelect) editSelect.innerHTML = '<option value="">Sin alumnos</option>';
        return;
      }
      const optionsHtml = alumnosState.map(a => `<option value="${a.uuid}">${a.nombre}</option>`).join('');
    ...
    function openEditReporteModal(id) {
      const rep = reportesState.find(r => String(r.id) === String(id));
      if (!rep) {
        showToast("Reporte no encontrado", "error");
        return;
      }
      populateReportesSelect();
      document.getElementById('edit_reporte_id').value = rep.id;
      document.getElementById('edit_reporte_alumno_uuid').value = rep.alumnoUuid;
      document.getElementById('edit_reporte_tipo').value = rep.tipo;
      document.getElementById('edit_reporte_fecha').value = rep.fecha || new Date().toISOString().split('T')[0];
      document.getElementById('edit_reporte_titulo').value = rep.titulo;
      document.getElementById('edit_reporte_desc').value = rep.desc;

      document.getElementById('modal-edit-reporte')?.classList.remove('hidden');
      lucide.createIcons();
    }

    function closeEditReporteModal() {
      document.getElementById('modal-edit-reporte')?.classList.add('hidden');
    }

    function handleUpdateReporte(e) {
      if (e) e.preventDefault();
      const id = document.getElementById('edit_reporte_id').value;
      const rep = reportesState.find(r => String(r.id) === String(id));
      if (!rep) return;

      rep.alumnoUuid = document.getElementById('edit_reporte_alumno_uuid').value;
      rep.tipo = document.getElementById('edit_reporte_tipo').value;
      rep.fecha = document.getElementById('edit_reporte_fecha').value;
      rep.titulo = document.getElementById('edit_reporte_titulo').value.trim();
      rep.desc = document.getElementById('edit_reporte_desc').value.trim();

      saveState();
      closeEditReporteModal();
      updateTeacherViews();

      if (currentParentStudent && currentParentStudent.uuid === rep.alumnoUuid) {
        renderParentPortal(currentParentStudent);
      }
          ...
          <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/80 p-4 shadow-xs space-y-2">
            <div class="flex items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-1.5">
                ...
                <button onclick="openEditReporteModal('${rep.id || idx}')" class="p-1 text-slate-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/40 rounded-lg cursor-pointer transition-colors" title="Editar reporte" aria-label="Editar reporte">
                  <i data-lucide="edit-3" class="w-3.5 h-3.5"></i>
                </button>
                <button onclick="deleteReporte(${idx})" class="p-1 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/40 rounded-lg cursor-pointer transition-colors" title="Eliminar" aria-label="Eliminar reporte">
                  <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
                </button>
              </div>
            </div>
            <h4 class="text-xs font-bold text-slate-800 dark:text-slate-200">${rep.titulo}</h4>
            <p class="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">${rep.desc}</p>
          </div>
        `;
      }).join('');
      lucide.createIcons();
    ...
    // ==========================================================
    // 10.2 GESTIÓN DE FIN DE CICLO ESCOLAR & OFERTA DE RENOVACIÓN ($200)
    // ==========================================================
    function openFinCicloModal() {
      const currentCiclo = maestroState.ciclo || '2026-2027';
      const parts = currentCiclo.split('-');
      let nextCiclo = '2027-2028';
      if (parts.length === 2) {
      ...
      const input = document.getElementById('nuevo_ciclo_nombre');
      if (input) input.value = nextCiclo;
      document.getElementById('modal-fin-ciclo')?.classList.remove('hidden');
      lucide.createIcons();
    }

    function closeFinCicloModal() {
      document.getElementById('modal-fin-ciclo')?.classList.add('hidden');
    }

    function descargarRespaldoCiclo() {
      exportFullBackupJSON();
    }

    function confirmarFinCicloYRenovacion() {
      const input = document.getElementById('nuevo_ciclo_nombre');
      const nuevoCiclo = input?.value?.trim() || '2027-2028';

      // 1. Descarga automática de respaldo preventivo
      exportFullBackupJSON();

      // 2. Limpieza de datos del ciclo que concluye
      alumnosState = [];
      proyectosState = [];
      tareasState = [];
      reportesState = [];
      anunciosState = [];
      calendarioEventsState = [];

      // 3. Configuración del nuevo ciclo escolar
      maestroState.ciclo = nuevoCiclo;

      // 4. Suscripción activa para nuevo ciclo con descuento de $50 MXN (cuota de $200 MXN en lugar de $250)
      maestroState.suscripcion = {
        estado: 'activa',
        plan: 'Docente Pro (Renovación Preferencial)',
        pagoInicial: SUSCRIPCION_RENOVACION_DESCUENTO, // $200 MXN con descuento de $50
        mensualidad: 50,
        fechaInicio: getTodayIsoDate(),
        proximoPago: '01 del Próximo Mes',
        ultimoPagoMonto: SUSCRIPCION_RENOVACION_DESCUENTO,
        mesesActivo: 1,
      ...
      const badgeCiclo = document.getElementById('badge-ciclo-actual');
      if (badgeCiclo) badgeCiclo.textContent = `Ciclo ${nuevoCiclo}`;
      const configCiclo = document.getElementById('config_ciclo');
      if (configCiclo) configCiclo.value = nuevoCiclo;

      showToast(`🎉 ¡Ciclo Escolar ${nuevoCiclo} iniciado! Se aplicó tu descuento de $50 MXN (Cuota preferencial: $200 MXN).`, "success");
    }
    ...
    function exportAsistenciasCSV() {
      ...
      let csv = `Nombre,Estado (${selectedAttendanceDate}),Hora,Presentes Acumulados,Retardos Acumulados,Faltas Acumuladas\n`;
      alumnosState.forEach(a => {
        recalculateStudentAttendanceTotals(a);
        const record = getStudentAttendanceForDate(a, selectedAttendanceDate);
        ...
        csv += `"${a.nombre}","${record.status}","${record.hora}",${p},${r},${f}\n`;
      });
      downloadCSV(csv, `asistencias_lumni_${selectedAttendanceDate}.csv`);
      showToast(`Reporte de asistencias (${selectedAttendanceDate}) exportado`, "success");
    }
    ...
    function exportCalificacionesCSV() {
      ...
      const trimName = selectedTrimester === 'anual' ? 'Anual' : `Trimestre_${selectedTrimester}`;
      ...
      alumnosState.forEach(a => {
        ...
        let row = `"${a.nombre}",`;
        materiasState.forEach(m => {
          const valNum = getStudentTrimesterGrade(a, selectedTrimester, m);
          const val = valNum !== null ? (selectedTrimester === 'anual' ? valNum.toFixed(1) : Math.round(valNum)) : '';
          if (valNum !== null) {
            sum += valNum;
            count++;
          }
      ...
      downloadCSV(csv, `calificaciones_lumni_${trimName}.csv`);
      showToast(`Reporte de calificaciones (${trimName}) exportado`, "success");
    }
    ...
    window.addEventListener('DOMContentLoaded', () => {
      // Inicializar selector interactivo de fecha de asistencia
      const dateInput = document.getElementById('asist-selected-date');
      if (dateInput) {
        dateInput.value = selectedAttendanceDate;
      }
      updateAttendanceDateBadge();

      // Inicializar badge de ciclo escolar
      const badgeCiclo = document.getElementById('badge-ciclo-actual');
      if (badgeCiclo) badgeCiclo.textContent = `Ciclo ${maestroState.ciclo || '2026-2027'}`;

      const loginCorreo = document.getElementById('login_correo');
      if (loginCorreo) loginCorreo.value = maestroState.correo;
      const regNombre = document.getElementById('reg_nombre');
      if (regNombre) regNombre.value = maestroState.nombre;
      const regCorreo = document.getElementById('reg_correo');
      if (regCorreo) regCorreo.value = maestroState.correo;
