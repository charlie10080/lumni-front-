...
export const ParentPortalView: React.FC<{ onNavigate: (tab: string) => void }> = ({ onNavigate }) => {
  ...
  const { students, subjects, activeTrimester, tasks, projects } = useData();
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-5 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Estado de Hoy (Viernes 11)</p>
            <div className="mt-2">
              <AttendanceBadge status="presente" />
            </div>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 flex items-center gap-1">
              <Clock className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" /> Ingreso: <strong className="text-slate-900 dark:text-white">07:54 AM</strong> (Puntual)
            </p>
          </div>
        ...
        <StatCard
          ...
          subtitle="Evaluación 1er Trimestre"
          icon={Award}
          colorVariant="amber"
        ...
        <StatCard
          ...
          value={`${student?.asistenciasTotales.presentes || 18} Sesiones`}
          subtitle="100% de asistencia este mes"
          icon={CalendarCheck}
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            <CardHeader className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-bold">Trimestre 1</span>
            </CardHeader>
                ...
                return (
                  <div key={sub.id} className="py-3 px-2 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/30 rounded-xl transition">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">Docente Titular</p>
                    </div>
          ...
          <Card>
            ...
            <div className="space-y-3 text-xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-slate-900 dark:text-white text-sm">Prof. Carlos Mendoza M.</p>
                  <p className="text-slate-500 dark:text-slate-400">Titular de 3er Grado Grupo B</p>
                </div>
              </div>
              <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-2 text-slate-600 dark:text-slate-300">
                <p className="flex items-center gap-2">
                  <Phone className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" /> (55) 8432-9011 ext. 302
                </p>
              </div>
              <Button
                variant="primary"
